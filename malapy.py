...
# Import the proteins to check

filepath = input("Enter the path to your CSV file with the list of proteins to annotate:  ")
annotated_df = pd.read_csv(filepath)

# Get the lists of diseases by category from MalaCards; include headers to prevent 403 error
...
def mala_checker(protein_name, output_type = "string"):
    ...
    results_df = pd.DataFrame(search_data, columns = search_cols)
    results_df = results_df.drop("Blank", axis = 1)
    results_list = results_df["Name"].values.tolist()

    results_count = len(results_list)
    delimeter = ", "
    results_string = delimeter.join(results_list)
